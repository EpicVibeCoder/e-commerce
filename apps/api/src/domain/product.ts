...
export class Product {
      ...
      constructor(props: { id: string; name: string; sku: string; price: string | number; stock: number; status: ProductStatus; categoryId: string }) {
            if (!props.name.trim()) {
            ...
            if (!Number.isInteger(props.stock) || props.stock < 0) {
                  throw new DomainError(`Stock must be a non-negative integer, got ${props.stock}`);
            }
      ...
      canFulfillQuantity(quantity: number): boolean {
            return Number.isInteger(quantity) && quantity > 0 && this.isActive() && this.stock >= quantity;
      }

      assertCanPurchase(quantity: number): void {
            if (!this.isActive()) {
                  throw new DomainError(`Product ${this.sku} is not available for purchase`);
            }
            if (!Number.isInteger(quantity) || quantity <= 0) {
                  throw new DomainError(`Quantity must be a positive integer, got ${quantity}`);
            }
            if (this.stock < quantity) {
                  throw new DomainError(`Insufficient stock for ${this.sku}: requested ${quantity}, available ${this.stock}`);
            }
      }
}
