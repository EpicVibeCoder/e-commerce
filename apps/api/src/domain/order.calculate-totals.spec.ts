...
describe("Order.calculateTotals", () => {
      ...
      it("accepts numeric unit prices and normalizes to two decimals", () => {
            const result = Order.calculateTotals([{ productId: "p1", quantity: 3, unitPrice: 9.99 }]);
      ...
      it("rejects empty line list", () => {
            ...
            expect(() => Order.calculateTotals([])).toThrow("Order must contain at least one line item");
      });

      it("rejects invalid quantity", () => {
            expect(() => Order.calculateTotals([{ productId: "p1", quantity: 0, unitPrice: "10.00" }])).toThrow(DomainError);
      });

      it("rejects non-positive unit price", () => {
            expect(() => Order.calculateTotals([{ productId: "p1", quantity: 1, unitPrice: "0.00" }])).toThrow(DomainError);
      });
});
