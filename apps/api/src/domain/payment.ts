...
const TERMINAL_STATUSES: ReadonlySet<PaymentStatus> = new Set([PaymentStatus.failed, PaymentStatus.cancelled, PaymentStatus.refunded]);

const ALLOWED_TRANSITIONS: ReadonlyMap<PaymentStatus, ReadonlySet<PaymentStatus>> = new Map([
      [PaymentStatus.pending, new Set([PaymentStatus.succeeded, PaymentStatus.failed, PaymentStatus.cancelled])],
...
export class Payment {
      ...
      constructor(props: { id: string; orderId: string; provider: PaymentProvider; transactionId: string; status: PaymentStatus }) {
      ...
      transitionTo(next: PaymentStatus): Payment {
            if (!this.canTransitionTo(next)) {
                  throw new DomainError(`Cannot transition payment from ${this.status} to ${next}`);
            }
            return new Payment({
                  id: this.id,
